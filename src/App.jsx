...
function TrustBadge({size=18,style={}}){
  return(
    <span style={{display:"inline-flex",alignItems:"center",justifyContent:"center",position:"relative",...style}}>
      <style>{`@keyframes trustPulse{0%,100%{opacity:1;transform:scale(1)}50%{opacity:0.8;transform:scale(1.08)}}`}</style>
      <svg width={size} height={size} viewBox="0 0 24 24" fill={G.gold} style={{animation:"trustPulse 2.4s ease-in-out infinite",filter:`drop-shadow(0 0 4px ${G.gold}88)`}}>
        <polygon points="12,2 15.5,8 22,9 17,14 18.5,21 12,17.5 5.5,21 7,14 2,9 8.5,8"/>
      </svg>
      <svg width={size*0.5} height={size*0.5} viewBox="0 0 24 24" fill="none" stroke="#000" strokeWidth="3.5" strokeLinecap="round" strokeLinejoin="round" style={{position:"absolute"}}>
...
function AdminPanel({st,update,addItem,removeItem,onClose}){
  ...
  const[cfg,setCfg]=useState(st.exchangeConfig||INIT.exchangeConfig);
  const imgRef=useRef();
  const TABS=["bias","events","news","notices","archive","kyc","trust+","config"];
      ...
      <div style={{padding:18}}>
        ...
        {tab==="config"&&(
          <div>
            ...
            <div style={{fontSize:11,color:G.textSub,marginBottom:18,lineHeight:1.6}}>Controls rate limits, fees, and admin payment accounts shown on the exchange.</div>
            ...
            <div style={{background:G.surface,border:`1px solid ${G.border}`,borderRadius:G.r,padding:16,marginBottom:14,display:"flex",justifyContent:"space-between",alignItems:"center"}}>
              <div>
                ...
                <div style={{fontSize:11,color:G.textSub}}>Toggle P2P exchange on or off</div>
              </div>
              <button onClick={()=>setCfg(c=>({...c,exchange_active:!c.exchange_active}))} style={{padding:"7px 16px",borderRadius:20,border:`1px solid ${cfg.exchange_active?G.green:G.red}`,background:cfg.exchange_active?G.greenBg:G.redBg,color:cfg.exchange_active?G.green:G.red,fontSize:12,fontWeight:700,cursor:"pointer",fontFamily:"inherit"}}>
            ...
            <div style={{background:G.surface,border:`1px solid ${G.border}`,borderRadius:G.r,padding:16,marginBottom:14}}>
              ...
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>
                {[["Min Rate","min_rate"],["Max Rate","max_rate"]].map(([l,k])=>(
                  <div key={k}>
                    <div style={{fontSize:10,color:G.textSub,marginBottom:5}}>{l}</div>
                    <input type="number" value={cfg[k]} onChange={e=>setCfg(c=>({...c,[k]:Number(e.target.value)}))}
            ...
            <div style={{background:G.surface,border:`1px solid ${G.border}`,borderRadius:G.r,padding:16,marginBottom:14}}>
              ...
              <div style={{marginBottom:10}}>
                <div style={{fontSize:10,color:G.textSub,marginBottom:5}}>Platform Fee (ETB) — paid by buyer to admin</div>
                <input type="number" value={cfg.platform_fee} onChange={e=>setCfg(c=>({...c,platform_fee:Number(e.target.value)}))}
                  style={{width:"100%",background:G.card,border:`1px solid ${G.border}`,borderRadius:G.rs,padding:"10px 12px",color:G.text,fontSize:14,fontWeight:700,outline:"none",boxSizing:"border-box",fontFamily:"inherit"}}/>
              </div>
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>
                {[["Min USDT","min_usdt"],["Max USDT","max_usdt"]].map(([l,k])=>(
                  <div key={k}>
                    <div style={{fontSize:10,color:G.textSub,marginBottom:5}}>{l}</div>
                    <input type="number" value={cfg[k]} onChange={e=>setCfg(c=>({...c,[k]:Number(e.target.value)}))}
            ...
            <div style={{background:G.surface,border:`1px solid ${G.border}`,borderRadius:G.r,padding:16,marginBottom:14}}>
              ...
              <div style={{fontSize:11,color:G.textSub,marginBottom:12,lineHeight:1.6}}>Buyers pay the platform fee to one of these accounts. Both fields required for each method.</div>
              {[["CBE","cbe"],["Telebirr","telebirr"]].map(([label,key])=>(
                <div key={key} style={{marginBottom:14}}>
                  <div style={{fontSize:11,color:G.text,fontWeight:700,marginBottom:7}}>{label}</div>
                  <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}}>
                    <div>
                      <div style={{fontSize:10,color:G.textSub,marginBottom:4}}>Account Number</div>
                      <input value={cfg[`admin_${key}_account`]} onChange={e=>setCfg(c=>({...c,[`admin_${key}_account`]:e.target.value}))}
                    ...
                    <div>
                      ...
                      <input value={cfg[`admin_${key}_name`]} onChange={e=>setCfg(c=>({...c,[`admin_${key}_name`]:e.target.value}))}
            ...
            <Btn onClick={()=>{update("exchangeConfig",cfg);alert("Config saved!");}} style={{width:"100%"}}>Save Config</Btn>
          </div>
        )}
    ...
    (async()=>{
      try{
        ...
        if(trRes.ok){
          ...
          // Rating
          let rating=null;
          const rRes=await fetch(`${SUPABASE_URL}/rest/v1/trade_ratings?rated_user_id=eq.${user.id}&select=rating`,{headers});
          if(rRes.ok){
            const ratings=await rRes.json();
            if(ratings.length>0) rating=(ratings.reduce((sum,r)=>sum+r.rating,0)/ratings.length).toFixed(1);
          }
