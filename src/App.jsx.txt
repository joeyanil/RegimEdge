...
const INIT = {
  weeklyBias:{ direction:"Bullish", conviction:"High", dayLabel:"Bullish Week", reasoning:"Gold remains in a strong macro uptrend. Real yields declining, DXY weakening below 98.5. Hold longs, avoid selling into strength this week.", keySupport:"3280", keyResistance:"3340", invalidation:"Break below 3220", tradeIdea:"", image:null, updatedAt:"Monday, May 5", updatedNote:"", postedAt:null },
  dailyBias:{ direction:"Bullish", dayLabel:"Bullish Day", body:"Intraday — Gold holding above 3280 support. Bias remains long for the session. Watch DXY for direction confirmation.", keyLevel:"", sessionBias:"", updatedAt:"Today, 08:00 AM", postedAt:null },
...
function PopupRotator({nav,onClose}){
  ...
  return(
    <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.6)",zIndex:200,display:"flex",alignItems:"flex-end",justifyContent:"center",padding:"0 16px 88px",backdropFilter:"blur(5px)"}}>
      <div style={{background:G.card,border:`1px solid ${s.color}44`,borderRadius:22,padding:"26px 22px 22px",width:"100%",maxWidth:420,position:"relative",boxShadow:`0 24px 60px rgba(0,0,0,0.7),0 0 40px ${s.color}14`}}>
        ...
        <button onClick={()=>{nav(s.page);onClose();}} style={{width:"100%",padding:14,background:G.gold,border:"none",borderRadius:G.rs,color:"#000",fontSize:13,fontWeight:800,cursor:"pointer",fontFamily:"inherit"}}>Go to {s.title} →</button>
      </div>
    </div>
  );
}
    ...
    <div>
      ...
      <div style={{padding:"28px 22px 0"}}>
          ...
          <div style={{marginBottom:26}}>
            ...
            {st.notices.slice(0,3).map(n=>(
              <div key={n.id} style={{background:G.surface,border:`1px solid ${G.border}`,borderRadius:G.rs,padding:"11px 14px",marginBottom:7,display:"flex",gap:12,alignItems:"flex-start"}}>
                <span style={{fontSize:16,flexShrink:0,marginTop:1}}>{n.icon||( n.type==="exchange"?"🏦":n.type==="alert"?"⚡":"📢")}</span>
                <div>
        ...
        <div style={{marginBottom:14}}>
          <WeeklyBiasCard wb={st.weeklyBias}/>
          <button onClick={()=>nav("weekly")} style={{width:"100%",padding:11,marginTop:8,background:"none",border:`1px solid ${st.weeklyBias.direction==="Bullish"?G.green:st.weeklyBias.direction==="Bearish"?G.red:G.gold}44`,borderRadius:G.rs,color:st.weeklyBias.direction==="Bullish"?G.green:st.weeklyBias.direction==="Bearish"?G.red:G.gold,fontSize:12,fontWeight:700,cursor:"pointer",fontFamily:"inherit"}}>Full Bias Analysis →</button>
        </div>
        ...
        <GlowCard color={st.dailyBias.direction==="Bullish"?G.green:G.red} style={{marginBottom:14}}>
          ...
          {st.dailyBias.keyLevel&&(
            <div style={{display:"flex",alignItems:"center",gap:8,padding:"7px 11px",background:G.surface,borderRadius:G.rs}}>
              <span style={{fontSize:10,color:G.textSub}}>Key Level:</span>
              <span style={{fontSize:13,color:G.gold,fontWeight:700}}>{st.dailyBias.keyLevel}</span>
            </div>
          )}
        </GlowCard>
        ...
        {(st.nfpSignal.active||st.fomcSignal.active)&&(
          <GlowCard color={G.gold} style={{marginBottom:14,cursor:"pointer"}} onClick={()=>nav("events")}>
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:11,marginBottom:34}}>
          {[
            ...
            <button key={page} onClick={()=>nav(page)} style={{background:G.card,border:`1px solid ${G.border}`,borderRadius:G.r,padding:"16px 14px",textAlign:"left",cursor:"pointer",transition:"all 0.2s"}}>
...
// ── WEEKLY BIAS CARD (reusable, used in WeeklyPage, HomePage, Admin Preview) ──
function WeeklyBiasCard({wb,db}){
  const c=wb.direction==="Bullish"?G.green:wb.direction==="Bearish"?G.red:G.gold;
  const convColor=wb.conviction==="High"?G.green:wb.conviction==="Medium"?G.gold:G.red;
  const timeAgo=(iso)=>{
    if(!iso)return"";
    const diff=Date.now()-new Date(iso);
    const h=Math.floor(diff/3600000); const m=Math.floor(diff/60000);
    if(h>48) return Math.floor(h/24)+"d ago";
    if(h>=1) return h+"h ago";
    return m+"m ago";
  };
  return(
    <div style={{background:`linear-gradient(135deg,${c}0a 0%,${G.card} 60%)`,border:`1px solid ${c}44`,borderRadius:G.r,overflow:"hidden",boxShadow:`0 0 32px ${c}18,inset 0 1px 0 ${c}18`}}>
      {/* Header */}
      <div style={{padding:"16px 18px 14px",borderBottom:`1px solid ${G.border}`}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:10}}>
          <div style={{fontSize:9,color:G.textSub,letterSpacing:2,textTransform:"uppercase"}}>Weekly Bias</div>
          <div style={{display:"flex",gap:6}}>
            <Badge color={c}>{wb.direction}</Badge>
            {wb.conviction&&<Badge color={convColor}>{wb.conviction}</Badge>}
          </div>
        </div>
        <div style={{fontFamily:"'Playfair Display',serif",fontSize:22,fontWeight:900,color:c,marginBottom:5}}>{wb.dayLabel}</div>
        <div style={{fontSize:11,color:G.textSub}}>
          {wb.updatedAt}{wb.postedAt&&<span style={{marginLeft:8,color:G.textDim}}>· {timeAgo(wb.postedAt)}</span>}
        </div>
      </div>
      {/* Reasoning */}
      <div style={{padding:"14px 18px",borderBottom:`1px solid ${G.border}`}}>
        <div style={{fontSize:9,color:G.textSub,letterSpacing:2,textTransform:"uppercase",marginBottom:8}}>Reasoning</div>
        <p style={{color:G.text,fontSize:13,lineHeight:1.85,margin:0}}>{wb.reasoning||wb.body||""}</p>
      </div>
      {/* Key Levels */}
      {(wb.keySupport||wb.keyResistance)&&(
        <div style={{padding:"14px 18px",borderBottom:`1px solid ${G.border}`}}>
          <div style={{fontSize:9,color:G.textSub,letterSpacing:2,textTransform:"uppercase",marginBottom:10}}>Key Levels</div>
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>
            {wb.keySupport&&(
              <div style={{background:G.greenBg,border:`1px solid ${G.green}33`,borderRadius:G.rs,padding:"12px 14px"}}>
                <div style={{fontSize:10,color:G.textSub,marginBottom:5}}>Support</div>
                <div style={{fontSize:18,fontWeight:900,color:G.green,fontFamily:"'Playfair Display',serif"}}>{wb.keySupport}</div>
              </div>
            )}
            {wb.keyResistance&&(
              <div style={{background:G.redBg,border:`1px solid ${G.red}33`,borderRadius:G.rs,padding:"12px 14px"}}>
                <div style={{fontSize:10,color:G.textSub,marginBottom:5}}>Resistance</div>
                <div style={{fontSize:18,fontWeight:900,color:G.red,fontFamily:"'Playfair Display',serif"}}>{wb.keyResistance}</div>
              </div>
            )}
          </div>
        </div>
      ...
      {wb.invalidation&&(
        <div style={{padding:"12px 18px",borderBottom:`1px solid ${G.border}`}}>
          <div style={{fontSize:9,color:G.textSub,letterSpacing:2,textTransform:"uppercase",marginBottom:6}}>Invalidation</div>
          <div style={{fontSize:13,color:G.red,fontWeight:600}}>{wb.invalidation}</div>
        </div>
      )}
      {/* Trade Idea */}
      {wb.tradeIdea&&(
        <div style={{padding:"12px 18px",borderBottom:`1px solid ${G.border}`}}>
          <div style={{fontSize:9,color:G.gold,letterSpacing:2,textTransform:"uppercase",marginBottom:6}}>Trade Idea</div>
          <p style={{color:G.text,fontSize:13,lineHeight:1.75,margin:0}}>{wb.tradeIdea}</p>
        </div>
      )}
      {/* Wednesday update */}
      {wb.updatedNote&&(
        <div style={{padding:"12px 18px",borderBottom:`1px solid ${G.border}`}}>
          <div style={{fontSize:12,color:G.gold,padding:"10px 14px",background:G.goldBg,borderRadius:8,borderLeft:`3px solid ${G.gold}`}}>Wednesday Update: {wb.updatedNote}</div>
        </div>
      )}
      {/* Chart image */}
      {wb.image&&<img src={wb.image} alt="chart" style={{width:"100%",display:"block"}}/>}
      {/* Footer */}
      <div style={{padding:"10px 18px"}}>
        <div style={{fontSize:11,color:G.textDim}}>Posted by RegimeEdge · {wb.updatedAt}</div>
      </div>
    </div>
  );
}
...
function WeeklyPage({st}){
  return(
    <div style={{padding:"32px 22px"}}>
      ...
      <GlowCard color={st.dailyBias.direction==="Bullish"?G.green:G.red} style={{marginBottom:16}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:10}}>
        ...
        <p style={{color:G.text,fontSize:13,lineHeight:1.8,margin:"0 0 10px"}}>{st.dailyBias.body}</p>
        {st.dailyBias.keyLevel&&(
          <div style={{display:"flex",alignItems:"center",gap:8,padding:"8px 12px",background:G.surface,borderRadius:G.rs,marginBottom:8}}>
            <span style={{fontSize:10,color:G.textSub}}>Key Level:</span>
            <span style={{fontSize:13,color:G.gold,fontWeight:700}}>{st.dailyBias.keyLevel}</span>
          </div>
        )}
        {st.dailyBias.sessionBias&&(
          <div style={{display:"flex",alignItems:"center",gap:8,padding:"8px 12px",background:G.surface,borderRadius:G.rs,marginBottom:8}}>
            <span style={{fontSize:10,color:G.textSub}}>Session Lean:</span>
            <span style={{fontSize:13,color:G.text,fontWeight:600}}>{st.dailyBias.sessionBias}</span>
          </div>
        )}
...
// ── MACRO DASHBOARD ───────────────────────────────────────────────────────────
// UPDATE THESE MANUALLY OR ADD ADMIN CONTROL
...
// ── EVENTS ────────────────────────────────────────────────────────────────────
function generateICS(title,countdownTo,desc){
  if(!countdownTo)return;
  const dt=new Date(countdownTo);
...
function EventsPage({st}){
  const UPCOMING=[
    { id:"nfp", title:"Non-Farm Payrolls", short:"NFP", date:"Jun 5, 2026", time:"12:30 UTC", color:G.gold, icon:"📊",
      ...
      countdownTo:"2026-06-05T12:30:00Z",
      sig: st.nfpSignal },
    { id:"fomc", title:"FOMC Rate Decision", short:"FOMC", date:"Jun 17–18, 2026", time:"18:00 UTC", color:G.blue, icon:"🏦",
      ...
      countdownTo:"2026-06-17T18:00:00Z",
      sig: st.fomcSignal },
    { id:"cpi", title:"US CPI Inflation", short:"CPI", date:"Jun 10, 2026", time:"12:30 UTC", color:"#f472b6", icon:"📈",
      ...
      countdownTo:"2026-06-10T12:30:00Z",
      sig: null },
    { id:"gdp", title:"US GDP (Q1 Final)", short:"GDP", date:"Jun 26, 2026", time:"12:30 UTC", color:G.textSub, icon:"🏛",
      ...
      countdownTo:"2026-06-26T12:30:00Z",
      sig: null },
  ];
        ...
        <div key={ev.id} style={{marginBottom:16}}>
          <div style={{background:G.card,border:`1px solid ${ev.color}33`,borderRadius:G.r,overflow:"hidden",
            ...
            <div style={{padding:"16px 18px"}}>
              ...
              <button onClick={()=>generateICS(ev.title,ev.sig?.countdownTo||ev.countdownTo,ev.desc)}
                style={{background:"none",border:`1px solid ${G.border}`,borderRadius:8,padding:"6px 12px",color:G.textSub,fontSize:11,cursor:"pointer",fontFamily:"inherit",display:"flex",alignItems:"center",gap:5,marginBottom:12}}>
                📅 Add to Calendar
              </button>
                  ...
                  {ev.sig.active&&ev.sig.prediction?(
                    <div>
                      ...
                      {ev.sig.result&&(
                        <div style={{
                          padding:"14px 16px",
                          background:ev.sig.result.toLowerCase().includes("correct")||ev.sig.result.toLowerCase().includes("paid")?G.greenBg:G.redBg,
                          border:`1px solid ${ev.sig.result.toLowerCase().includes("correct")||ev.sig.result.toLowerCase().includes("paid")?G.green+"44":G.red+"44"}`,
                          borderRadius:G.rs,marginTop:10
                        }}>
                          <div style={{fontSize:9,letterSpacing:2,fontWeight:700,color:ev.sig.result.toLowerCase().includes("correct")||ev.sig.result.toLowerCase().includes("paid")?G.green:G.red,marginBottom:6}}>POST-EVENT RESULT</div>
                          <div style={{fontSize:13,color:G.text,lineHeight:1.7}}>{ev.sig.result}</div>
                        </div>
                      )}
                    </div>
                  ):(
...
function TerminalFull(){
  ...
  useEffect(()=>{const t=setTimeout(()=>setInitializing(false),600);return()=>clearTimeout(t);},[]);
  ...
  const BASE_URL = import.meta.env.VITE_CAPITAL_ENV === "live"
  ...
  if(initializing) return(
    <div style={{display:"flex",flexDirection:"column",height:"calc(100vh - 140px)",fontFamily:M,background:G.bgDeep}}>
      {/* Skeleton header */}
      <div style={{padding:"12px 16px 10px",borderBottom:`1px solid ${G.border}`,display:"flex",alignItems:"center",justifyContent:"space-between"}}>
        <div>
          <div style={{width:140,height:8,background:G.border,borderRadius:4,marginBottom:8,animation:"pulse 1.4s ease-in-out infinite"}}/>
          <div style={{width:100,height:14,background:G.border,borderRadius:4,animation:"pulse 1.4s ease-in-out infinite"}}/>
        </div>
        <div style={{width:72,height:32,background:G.border,borderRadius:G.rs,animation:"pulse 1.4s ease-in-out infinite"}}/>
      </div>
      {/* Skeleton tab bar */}
      <div style={{display:"flex",gap:8,padding:"10px 16px",borderBottom:`1px solid ${G.border}`}}>
        {[80,90,70,60,75].map((w,i)=>(
          <div key={i} style={{width:w,height:28,background:G.border,borderRadius:G.rs,animation:"pulse 1.4s ease-in-out infinite",animationDelay:`${i*0.1}s`}}/>
        ))}
      </div>
      {/* Skeleton cards */}
      <div style={{flex:1,overflowY:"auto",padding:16,display:"flex",flexDirection:"column",gap:12}}>
        {[1,2,3].map(i=>(
          <div key={i} style={{background:G.card,borderRadius:G.r,padding:20,border:`1px solid ${G.border}`}}>
            <div style={{width:"60%",height:10,background:G.border,borderRadius:4,marginBottom:14,animation:"pulse 1.4s ease-in-out infinite"}}/>
            <div style={{width:"100%",height:8,background:G.border,borderRadius:4,marginBottom:8,animation:"pulse 1.4s ease-in-out infinite"}}/>
            <div style={{width:"80%",height:8,background:G.border,borderRadius:4,animation:"pulse 1.4s ease-in-out infinite"}}/>
          </div>
        ))}
      </div>
...
function AdminPanel({st,update,addItem,removeItem,onClose}){
  ...
  const[no,setNo]=useState({text:"",type:"announcement",icon:"📢"});
  const[aw,setAw]=useState({week:"",bias:"Bullish",result:"green",note:""});
  const[editingNewsId,setEditingNewsId]=useState(null);
  const[deleteConfirmId,setDeleteConfirmId]=useState(null);
  const[showBiasPreview,setShowBiasPreview]=useState(false);
  const imgRef=useRef();
  ...
  const ConvictionPicker=({val,onChange})=>(
    <div style={{display:"flex",gap:7,marginBottom:14}}>
      {["High","Medium","Low"].map(c=>{
        const cc=c==="High"?G.green:c==="Medium"?G.gold:G.red;
        return(
          <button key={c} onClick={()=>onChange(c)} style={{flex:1,padding:9,borderRadius:9,border:`1px solid ${val===c?cc:G.border}`,background:val===c?`${cc}15`:"none",color:val===c?cc:G.textSub,fontSize:12,fontWeight:700,cursor:"pointer",fontFamily:"inherit"}}>{c}</button>
        );
      })}
    </div>
  );
    ...
    <div style={{minHeight:"100vh",background:G.bgDeep,paddingBottom:40}}>
      ...
      <div style={{padding:18}}>
        ...
        {tab==="bias"&&<>
          ...
          <div style={{fontSize:11,color:G.textSub,marginBottom:6}}>Direction</div>
          <DB val={wb.direction} onChange={d=>setWb(b=>({...b,direction:d,dayLabel:`${d} Week`}))}/>
          <div style={{fontSize:11,color:G.textSub,marginBottom:6}}>Conviction</div>
          <ConvictionPicker val={wb.conviction||"High"} onChange={c=>setWb(b=>({...b,conviction:c}))}/>
          <FTA value={wb.reasoning||wb.body||""} onChange={v=>setWb(b=>({...b,reasoning:v,body:v}))} placeholder="Market reasoning — macro context, why this direction" rows={4}/>
          <div style={{height:9}}/>
          <FI value={wb.keySupport||""} onChange={v=>setWb(b=>({...b,keySupport:v}))} placeholder="Key support level e.g. 3280" style={{marginBottom:9}}/>
          <FI value={wb.keyResistance||""} onChange={v=>setWb(b=>({...b,keyResistance:v}))} placeholder="Key resistance level e.g. 3340" style={{marginBottom:9}}/>
          <FI value={wb.invalidation||""} onChange={v=>setWb(b=>({...b,invalidation:v}))} placeholder="Invalidation — what would change the bias" style={{marginBottom:9}}/>
          <FTA value={wb.tradeIdea||""} onChange={v=>setWb(b=>({...b,tradeIdea:v}))} placeholder="Trade idea / setup to watch (optional)" rows={2}/>
          <div style={{height:9}}/>
          ...
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:9,marginBottom:0}}>
            <button onClick={()=>setShowBiasPreview(true)} style={{padding:13,background:G.surface,border:`1px solid ${G.border}`,borderRadius:G.rs,color:G.text,fontSize:13,fontWeight:700,cursor:"pointer",fontFamily:"inherit"}}>Preview</button>
            <Btn onClick={()=>{update("weeklyBias",{...wb,postedAt:new Date().toISOString()});alert("Weekly bias saved!");}}>Save & Publish</Btn>
          </div>
          <Div/>
          ...
          <FTA value={db.body} onChange={v=>setDb(b=>({...b,body:v}))} placeholder="Session context — what to watch today" rows={3}/>
          <div style={{height:9}}/>
          <FI value={db.keyLevel||""} onChange={v=>setDb(b=>({...b,keyLevel:v}))} placeholder="Key level for today e.g. 3295 support" style={{marginBottom:9}}/>
          <FI value={db.sessionBias||""} onChange={v=>setDb(b=>({...b,sessionBias:v}))} placeholder="London / NY session lean (optional)" style={{marginBottom:9}}/>
        ...
        {tab==="news"&&<>
          <div style={{fontSize:13,color:G.text,fontWeight:700,marginBottom:10}}>{editingNewsId?"Edit News":"Post News"}</div>
          ...
          <div style={{display:"flex",gap:9,marginBottom:22}}>
            {editingNewsId&&<Btn variant="outline" onClick={()=>{setEditingNewsId(null);setNn({headline:"",take:"",tag:"Gold"});}} style={{flex:1}}>Cancel</Btn>}
            <Btn onClick={()=>{
              if(!nn.headline)return;
              if(editingNewsId){
                update("news",st.news.map(n=>n.id===editingNewsId?{...nn,id:editingNewsId}:n));
                setEditingNewsId(null);
                alert("News updated!");
              } else {
                addItem("news",{...nn,id:Date.now(),time:"Just now"});
                alert("News posted!");
              }
              setNn({headline:"",take:"",tag:"Gold"});
            }} style={{flex:1}}>{editingNewsId?"Update News":"Post News"}</Btn>
          </div>
          {st.news.map(n=>(
            <div key={n.id} style={{background:G.surface,border:`1px solid ${editingNewsId===n.id?G.gold:G.border}`,borderRadius:9,padding:11,marginBottom:7,display:"flex",justifyContent:"space-between",alignItems:"center"}}>
              <button onClick={()=>{setEditingNewsId(n.id);setNn({headline:n.headline,take:n.take,tag:n.tag||"Gold"});}} style={{fontSize:12,color:G.text,flex:1,marginRight:9,lineHeight:1.5,background:"none",border:"none",cursor:"pointer",textAlign:"left",fontFamily:"inherit",padding:0}}>{n.headline}</button>
        ...
        {tab==="notices"&&<>
          ...
          <div style={{height:9}}/>
          <div style={{fontSize:11,color:G.textSub,marginBottom:6}}>Type</div>
          <div style={{display:"flex",gap:7,marginBottom:11}}>
            {["announcement","exchange","alert"].map(t=>(
              <button key={t} onClick={()=>setNo(n=>({...n,type:t}))} style={{flex:1,padding:8,borderRadius:8,border:`1px solid ${no.type===t?G.gold:G.border}`,background:no.type===t?G.goldBg:"none",color:no.type===t?G.gold:G.textSub,fontSize:10,fontWeight:700,cursor:"pointer",fontFamily:"inherit",textTransform:"capitalize"}}>{t}</button>
            ))}
          </div>
          <div style={{fontSize:11,color:G.textSub,marginBottom:8}}>Icon</div>
          <div style={{display:"flex",gap:7,marginBottom:13}}>
            {NOTICE_ICONS.map(ic=>(
              <button key={ic} onClick={()=>setNo(n=>({...n,icon:ic}))} style={{width:38,height:38,borderRadius:9,border:`1px solid ${no.icon===ic?G.gold:G.border}`,background:no.icon===ic?G.goldBg:"none",fontSize:18,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center"}}>{ic}</button>
            ))}
          </div>
          <Btn onClick={()=>{if(!no.text)return;addItem("notices",{...no,id:Date.now(),time:"Just now"});setNo({text:"",type:"announcement",icon:"📢"});alert("Notice posted!");}} style={{width:"100%",marginBottom:22}}>Post Notice</Btn>
          {st.notices.map(n=>(
            <div key={n.id} style={{background:G.surface,border:`1px solid ${G.border}`,borderRadius:9,padding:11,marginBottom:7,display:"flex",justifyContent:"space-between",alignItems:"center"}}>
              <div style={{display:"flex",gap:9,alignItems:"center",flex:1,marginRight:9}}>
                <span style={{fontSize:16}}>{n.icon||"📢"}</span>
                <span style={{fontSize:12,color:G.text}}>{n.text}</span>
              </div>
            ...
            <div key={w.id} style={{background:G.surface,border:`1px solid ${G.border}`,borderRadius:9,padding:11,marginBottom:7,display:"flex",justifyContent:"space-between",alignItems:"center"}}>
              ...
              <Btn variant="danger" onClick={()=>{
                if(deleteConfirmId===w.id){
                  removeItem("archiveWeeks",w.id);
                  setDeleteConfirmId(null);
                } else {
                  setDeleteConfirmId(w.id);
                  setTimeout(()=>setDeleteConfirmId(null),3000);
                }
              }} style={{padding:"5px 9px",fontSize:11}}>{deleteConfirmId===w.id?"Confirm?":"✕"}</Btn>
            </div>
          ))}
        </>}
      ...
      {/* Bias Preview Modal */}
      {showBiasPreview&&(
        <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.92)",zIndex:300,overflowY:"auto",padding:"20px 16px 40px"}}>
          <div style={{maxWidth:480,margin:"0 auto"}}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:16}}>
              <div style={{fontFamily:"'Playfair Display',serif",fontSize:16,color:G.gold}}>Preview</div>
              <button onClick={()=>setShowBiasPreview(false)} style={{background:"none",border:`1px solid ${G.border}`,borderRadius:8,color:G.textSub,cursor:"pointer",fontSize:13,padding:"5px 12px",fontFamily:"inherit"}}>← Back to edit</button>
            </div>
            <WeeklyBiasCard wb={wb}/>
            <div style={{height:16}}/>
            <Btn onClick={()=>{update("weeklyBias",{...wb,postedAt:new Date().toISOString()});setShowBiasPreview(false);alert("Weekly bias saved!");}} style={{width:"100%"}}>Save & Publish</Btn>
          </div>
        </div>
      )}
    </div>
    ...
    <div style={{padding:"32px 22px"}}>
      ...
      {tab==="profile"&&(
        <div>
          ...
          {/* EA Terminal access — prominent status card */}
          <div style={{
            background:isApproved?G.greenBg:G.surface,
            border:`1px solid ${isApproved?G.green+"44":G.border}`,
            borderRadius:G.r,padding:18,marginBottom:16
          }}>
            <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:isApproved?0:14}}>
              <span style={{fontSize:20}}>{isApproved?"✅":"⏳"}</span>
              <div>
                <div style={{fontSize:13,fontWeight:700,color:G.text}}>
                  {isApproved?"EA Terminal — Active":"EA Terminal — Pending Approval"}
                </div>
                <div style={{fontSize:11,color:G.textSub,marginTop:2}}>
                  {isApproved?"You have full access to EdgeTerminal":"One-time admin approval needed"}
                </div>
              </div>
              {isApproved&&<span style={{marginLeft:"auto",padding:"5px 12px",borderRadius:20,background:"rgba(34,197,94,0.12)",border:`1px solid ${G.green}44`,color:G.green,fontSize:10,fontWeight:700,flexShrink:0}}>APPROVED ✓</span>}
            </div>
            {!isApproved&&(
              <a href={ADMIN_TG} target="_blank" rel="noreferrer" style={{
                display:"block",padding:11,background:G.gold,borderRadius:G.rs,
                color:"#000",textAlign:"center",fontSize:12,fontWeight:800,textDecoration:"none"
              }}>Request Access on Telegram →</a>
            )}
          </div>
...
export default function App(){
  ...
  // ── PAGE — hash-based routing for persistence on reload ──────────────────────
  ...
  const nav=p=>{
    ...
    setOpenGroup(null);
    setShowProfileMenu(false);
    // Hash routing: update URL bar without a server request
